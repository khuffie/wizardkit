...
public class WMSTileOverlay : MKTileOverlay {
     ...
     */
	public init(urlArg: String, useMercator: Bool, wmsVersion: String, name: String = "Default") {
        self.url = urlArg
        self.useMercator = useMercator
        self.wmsVersion = wmsVersion
		self.name = name
		self.TILE_CACHE = "TILE_CACHE-\(name)"
    ...
    public override func loadTile(at path: MKTileOverlayPath, result: @escaping (Data?, Error?) -> Void) {
		//print("WizardKit.loadTile for \(name)")
        ...
        if file.fileExists(atPath: filePath) {
            ...
            session.dataTask(with: request as URLRequest, completionHandler: {(data, response, error) in
                ...
                if error != nil {
                    print("WizardKit.Error downloading tile")
                    result(nil, error)
                }
                else {
                    do {
